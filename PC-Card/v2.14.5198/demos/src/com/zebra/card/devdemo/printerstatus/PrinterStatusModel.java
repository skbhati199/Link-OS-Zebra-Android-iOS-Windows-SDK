...
public class PrinterStatusModel extends PrinterModel {
	...
	private String[][] mapMediaInfoToNameValueList(List<MediaInfo> mediaInfo) {
		...
		for (MediaInfo info : mediaInfo) {
			result[counter++] = new String[] { info.type.name(), Integer.toString(info.initialSize), Integer.toString(info.panelsRemaining), info.description, info.oemCode, info.partNumber };
		}
