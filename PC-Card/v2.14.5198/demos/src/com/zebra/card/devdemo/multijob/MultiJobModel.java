...
public class MultiJobModel extends PrintModel {
	...
	private void setJobSetting(Job job, JobSettingOptions printOptions) throws SettingsException, ConnectionException {
		...
		if (printOptions.isMagneticEncodingShown()) {
		...
		if (printOptions.isPrintOptimizationShown()) {
	...
	private void sendPrintJob(Job job) throws IOException, SettingsException, ZebraCardException, ConnectionException, NumberFormatException {
		...
		if (frontCheckBox || backCheckBox || magEncodeCheckBox) {
			...
			printJobOptions.setTrack1Data(job.track1DataTextField.getText());
			printJobOptions.setTrack2Data(job.track2DataTextField.getText());
			printJobOptions.setTrack3Data(job.track3DataTextField.getText());
			printJobOptions.setQuantity((Integer) job.quantityComboBox.getSelectedItem());

			if (frontCheckBox || backCheckBox) {
				printJobOptions.setFrontImageInfo(job.frontImageInfo);
				printJobOptions.setBackImageInfo(job.backImageInfo);
				printJobOptions.setFrontSelected(job.frontCheckBox.isSelected());
				printJobOptions.setBackSelected(job.backCheckBox.isSelected());
			...
			jobInfoList.add(new MultiJobInfo(CardSource.fromString((String) job.sourceComboBox.getSelectedItem()), jobId, job));
		}
	}

	public int magEncode(PrintJobOptions printJobOptions) throws ConnectionException, SettingsException, ZebraCardException {
		return getZebraCardPrinter().magEncode(printJobOptions.getQuantity(), printJobOptions.getTrack1Data(), printJobOptions.getTrack2Data(), printJobOptions.getTrack3Data());
	}
	...
	public int printAndMagEncode(PrintJobOptions printJobOptions) throws IOException, ConnectionException, SettingsException, ZebraCardException {
		...
		try {
			...
			if (printJobOptions.isFrontSelected()) {
				graphicsInfo.addAll(createGraphicsInfo(graphics, printJobOptions.getFrontImageInfo(), CardSide.Front));
			}

			if (printJobOptions.isBackSelected()) {
				graphicsInfo.addAll(createGraphicsInfo(graphics, printJobOptions.getBackImageInfo(), CardSide.Back));
			}

			if (printJobOptions.hasTrackData()) {
				return getZebraCardPrinter().printAndMagEncode(printJobOptions.getQuantity(), graphicsInfo, printJobOptions.getTrack1Data(), printJobOptions.getTrack2Data(), printJobOptions.getTrack3Data());
			} else {
				return getZebraCardPrinter().print(printJobOptions.getQuantity(), graphicsInfo);
			}
		} finally {
			if (graphics != null) {
				graphics.close();
	...
	private List<GraphicsInfo> createGraphicsInfo(ZebraCardGraphics graphics, Map<PrintType, String> imageInfo, CardSide side) throws IOException {
		...
		for (PrintType type : imageInfo.keySet()) {
			...
			if (type.equals(PrintType.Overlay) && imageInfo.get(type) == null) {
				...
				graphicsInfo.add(addImage(graphics.createImage(), side, type));
			}
