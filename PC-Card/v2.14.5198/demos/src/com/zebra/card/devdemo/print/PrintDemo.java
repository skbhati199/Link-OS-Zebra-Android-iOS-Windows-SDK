...
public class PrintDemo extends PrinterDemoBase<PrintModel> {
				...
				new Thread(new Runnable() {
					...
					public void run() {
						...
						if (selectPrintFrontCheckBox.isSelected()) {
							...
							printJobOptions.getFrontImageInfo().put(printType, frontSideImageFile);
						}
						...
						if (selectPrintOverlayCheckBox.isSelected()) {
							...
							printJobOptions.getFrontImageInfo().put(PrintType.Overlay, overlaySideImageFile);
						}

						String backSideImageFile = null;
						if (selectPrintBackCheckBox.isSelected()) {
							backSideImageFile = backImageFileNameTextField.getText();
							printJobOptions.getBackImageInfo().put(PrintType.MonoK, backSideImageFile);
						}

						printJobOptions.setQuantity(Integer.parseInt((String) printQuantityComboBox.getSelectedItem()));
		...
		frontImageBrowseButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				if (FileHelper.getBmpFileChooser().showOpenDialog(demoDialog) == JFileChooser.APPROVE_OPTION) {
					frontImageFileNameTextField.setText(FileHelper.getBmpFileChooser().getSelectedFile().getAbsolutePath());
				}
			}
		});
	...
	private JPanel createFrontOverlayPanel() {
		...
		browseButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				if (FileHelper.getBmpFileChooser().showOpenDialog(demoDialog) == JFileChooser.APPROVE_OPTION) {
					overlayImageFileNameTextField.setText(FileHelper.getBmpFileChooser().getSelectedFile().getAbsolutePath());
				}
			}
		});
	...
	private JPanel createBackMonoPanel() {
		...
		browseButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				if (FileHelper.getBmpFileChooser().showOpenDialog(demoDialog) == JFileChooser.APPROVE_OPTION) {
					backImageFileNameTextField.setText(FileHelper.getBmpFileChooser().getSelectedFile().getAbsolutePath());
				}
			}
		});
