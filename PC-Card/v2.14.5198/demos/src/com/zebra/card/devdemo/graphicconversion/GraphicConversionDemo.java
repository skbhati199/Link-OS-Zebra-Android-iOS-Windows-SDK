...
public class GraphicConversionDemo extends PrinterDemoBase<GraphicConversionModel> {
	...
	private static final String[] GRAPHICS_FORMAT_LIST = new String[] { GraphicConverter.GRAPHICS_FORMAT_COLOR, GraphicConverter.GRAPHICS_FORMAT_MONO_DIFFUSION, GraphicConverter.GRAPHICS_FORMAT_MONO_HALFTONE_6X6, GraphicConverter.GRAPHICS_FORMAT_MONO_HALFTONE_8X8,
			GraphicConverter.GRAPHICS_FORMAT_GRAY_DIFFUSION, GraphicConverter.GRAY_HALFTONE_6X6, GraphicConverter.GRAY_HALFTONE_8X8 };

	private static final PrinterModelInfo[] PRINTER_MODEL_INFO_LIST = new PrinterModelInfo[] { PrinterModelInfo.ZXP1, PrinterModelInfo.ZXP3, PrinterModelInfo.ZXP7, PrinterModelInfo.ZXP8, PrinterModelInfo.ZXP9, PrinterModelInfo.ZC100, PrinterModelInfo.ZC150,
			PrinterModelInfo.ZC300, PrinterModelInfo.ZC350 };
	...
	private GraphicsContainer buildGraphicsContainer() {
		...
		graphicsContainer.setFormat((String) graphicsFormatComboBox.getSelectedItem());
		graphicsContainer.setOutputFilePath(convertedFileNameTextField.getText() + ".bmp");
		graphicsContainer.setInputFilePath(fileNameTextField.getText());
		graphicsContainer.setPrinterModelInfo((PrinterModelInfo) printerModelComboBox.getSelectedItem());
		graphicsContainer.setWidth(widthTextField.getText());
		graphicsContainer.setHeight(heightTextField.getText());
		graphicsContainer.setXOffset(xOffsetTextField.getText());
		graphicsContainer.setYOffset(yOffsetTextField.getText());
		graphicsContainer.setDimensionOption(getDimensionOption());
	...
	private JPanel createOutputFilenamePanel() {
		...
		fileNameArea.add(new JLabel("Filename of Converted Graphic"));
	...
	private JPanel createFilenamePanel() {
		...
		browseButton.addActionListener(new ActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent arg0) {
				if (FileHelper.getImageFileChooser().showOpenDialog(demoDialog) == JFileChooser.APPROVE_OPTION) {
					fileNameTextField.setText(FileHelper.getImageFileChooser().getSelectedFile().getAbsolutePath());
				}
			}
		});
	...
	private Component createOptionsPanel() {
		...
		graphicsFormatComboBox = new JComboBox(GRAPHICS_FORMAT_LIST);
		...
		printerModelComboBox = new JComboBox(PRINTER_MODEL_INFO_LIST);
