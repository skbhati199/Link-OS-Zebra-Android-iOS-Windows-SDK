...
import com.zebra.card.devdemo.utils.ThreadSleeper;
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (!jobId.isEmpty()) {
					try {
						...
						statusTextArea.setText(statusTextArea.getText() + "Invalid entry. Job ID must be a whole number.\n");
		...
		try {
			...
			if (installedRibbon != null && !installedRibbon.isEmpty()) {
				...
				if (installedRibbon.contains(COLOR_OPTION)) {
					printOptions.setColorAllowed(true);
				}

				if (isPrintTypeSupported(installedRibbon, monoRibbonOptions)) {
					printOptions.setMonoAllowed(true);
				}

				if (isPrintTypeSupported(installedRibbon, overlayRibbonOptions)) {
					printOptions.setOverlayAllowed(true);
				}

				printOptions.setCardSourceRange(getPrinterModel().getZebraCardPrinter().getJobSettingRange(ZebraCardJobSettingNames.CARD_SOURCE));
				printOptions.setCardDestinationRange(getPrinterModel().getZebraCardPrinter().getJobSettingRange(ZebraCardJobSettingNames.CARD_DESTINATION));
				printOptions.setLaminatorDestinationsShown(getPrinterModel().getZebraCardPrinter().hasLaminator());
