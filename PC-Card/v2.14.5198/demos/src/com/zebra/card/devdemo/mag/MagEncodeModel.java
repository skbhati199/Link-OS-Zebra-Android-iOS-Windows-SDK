...
public class MagEncodeModel extends PrinterModel {
	...
	public void magEncode(MagEncodeContainer container, final JTextArea jobStatusArea) throws ConnectionException, SettingsException, ZebraCardException {
		...
		jobSettings.put(ZebraCardJobSettingNames.CARD_SOURCE, container.getCardSource());
		jobSettings.put(ZebraCardJobSettingNames.CARD_DESTINATION, container.getCardDestination());
		jobSettings.put(ZebraCardJobSettingNames.MAG_COERCIVITY, container.getCoercivityType());
		jobSettings.put(ZebraCardJobSettingNames.MAG_VERIFY, container.isEncodingVerified() ? "yes" : "no");

		getZebraCardPrinter().setJobSettings(jobSettings);

		int jobId = getZebraCardPrinter().magEncode(container.getQuantity(), container.getTrack1Data(), container.getTrack2Data(), container.getTrack3Data());
		new PollJobStatusWorker(getZebraCardPrinter(), new JobInfo(jobId, CardSource.fromString(container.getCardSource()))) {
			@Override
