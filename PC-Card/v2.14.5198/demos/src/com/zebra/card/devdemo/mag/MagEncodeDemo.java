...
public class MagEncodeDemo extends PrinterDemoBase<MagEncodeModel> {
	...
	private void setUpMagDemo() {
		...
		try {
			...
			if (hasMagEncoder) {
				...
				enableMagEncoding(false);
				throw new Exception("The selected printer does not have a magnetic encoder.\nPlease choose a different printer from the list.");
			}
		} catch (Exception e) {
			JOptionPane.showMessageDialog(null, e.getLocalizedMessage());
		} finally {
	...
	private MagEncodeContainer buildMagEncodeContainer() {
		MagEncodeContainer magEncodeContainer = new MagEncodeContainer();
		magEncodeContainer.setCardSource((String) cardSourceComboBox.getSelectedItem());
		magEncodeContainer.setCardDestination((String) cardDestinationComboBox.getSelectedItem());
		magEncodeContainer.setCoercivityType((String) coercivityTypeComboBox.getSelectedItem());
		magEncodeContainer.setEncodingVerified(verifyCheckBox.isSelected());
		magEncodeContainer.setTrack1Data(track1DataTextField.getText());
		magEncodeContainer.setTrack2Data(track2DataTextField.getText());
		magEncodeContainer.setTrack3Data(track3DataTextField.getText());
		magEncodeContainer.setQuantity((Integer) quantityComboBox.getSelectedItem());
		return magEncodeContainer;
	}
}
